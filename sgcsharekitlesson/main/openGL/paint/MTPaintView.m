...
//    glHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST);
    ...
    _program = [MTShaderOperations compileShaders:@"MTPaintVertex"
                                   shaderFragment:@"MTPaintFragment"];
    
    glUseProgram(_program);
    _positionSlot = glGetAttribLocation(_program, "Position");
    _textureSlot = glGetUniformLocation(_program, "Texture");
    _textureCoordsSlot = glGetAttribLocation(_program, "TextureCoords");
        _textureSlotMask = glGetUniformLocation(_program, "TextureMask");
        _textureCoordsSlotMask = glGetAttribLocation(_program, "TextureCoordsMask");
   GLuint  _colorSlot = glGetUniformLocation(_program, "SourceColor");
    ...
    glUniform1i(_textureSlot, 5);
        ...
        glBindTexture(GL_TEXTURE_2D, _textureIDMask);
        glUniform1i(_textureSlotMask, 0);
...
- (void)drawCGPointsViaOpenGLES:(NSArray *)points inFrame:(CGRect)rect {
    ...
    for (NSValue *value in points) {
        ...
        glVertexAttribPointer(_textureCoordsSlotMask, 2, GL_FLOAT, GL_FALSE, 0, texCoordsMask);
        glEnableVertexAttribArray(_textureCoordsSlotMask);
